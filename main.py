...
startprogram = "Excuting machine program"
cursor = db.cursor()
...
if list[0,1] == list[0,1]:
    var3.set(1)
    print("Send it to the cloud (data bank) for analysis")
    os.environ['TNS_ADMIN'] = r"C:\Users\%s\Desktop\RFID\Wallet_DB202005021424"%(username)
    db = cx_Oracle.connect('ADMIN', 'Prudhvi@1997', 'db202005021424_low')
    cursor = db.cursor()
    try:
...
if list[0,1] == list[0,1]:
    var4.set(1)
    print(startprogram)
    print("Receive the Operations sequence data")
    os.environ['TNS_ADMIN'] = r"C:\Users\%s\Desktop\RFID\Wallet_DB202005021424"%(username)
    db = cx_Oracle.connect('ADMIN', 'Prudhvi@1997', 'db202005021424_low')
    cursor = db.cursor()
    try:
